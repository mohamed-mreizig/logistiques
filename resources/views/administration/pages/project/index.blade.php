...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 class="text-xl font-semibold text-gray-800">Projets</h2>
            @can('create user')
            <a href="{{ url('projet/create') }}" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                Ajouter un projet
            </a>
            @endcan
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow">
                    <img src="{{ s3Asset($document->imgUrl) }}" alt="{{ $document->titleFr }}" class="w-full h-48 object-cover">
                    <div class="p-4">
                        ...
                        <div class="prose max-h-24 overflow-hidden text-ellipsis text-gray-600 mb-4">
                            {!! preg_replace('/<div[^>]*>|<\/div>/', '', $document->descriptionFR) !!}
                        </div>
                        <div class="flex space-x-2">
                            <a href="{{ url('projet/' . $document->id) }}" 
                               class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition-colors">
                               Modifier
                            </a>
                            <a href="{{ url('projet/'.$document->id.'/delete') }}" 
