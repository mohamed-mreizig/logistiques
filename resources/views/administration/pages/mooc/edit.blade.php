...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Modifier le MOOC</h2>
        ...
        <div class="p-6">
            <form action="{{ route('mooc.update', $mooc->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="titleFr" id="titleFr" value="{{ old('titleFr', $mooc->titleFr) }}" 
                    ...
                    <div>
                        ...
                        <input type="text" name="titleAr" id="titleAr" value="{{ old('titleAr', $mooc->titleAr) }}" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                        @error('titleAr')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <div class="border border-gray-300 rounded-md">
                        <textarea name="descriptionFr" id="myeditorinstance" class="w-full px-3 py-2 min-h-[200px]">{{ old('descriptionFr', $mooc->descriptionFr) }}</textarea>
                    </div>
                    @error('descriptionFr')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="border border-gray-300 rounded-md">
                        <textarea name="descriptionAr" id="myeditorinstance" class="w-full px-3 py-2 min-h-[200px]">{{ old('descriptionAr', $mooc->descriptionAr) }}</textarea>
                    </div>
                    @error('descriptionAr')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @if($mooc->imageUrl)
                            <div class="mt-2">
                                <img src="{{ s3Asset($mooc->imageUrl) }}" class="h-20 w-auto rounded-md border border-gray-200">
                            </div>
                        @endif
                        @error('imageUrl')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
