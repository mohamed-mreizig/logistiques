...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h2 class="text-xl font-semibold text-gray-800">Modifier le document</h2>
        ...
        <div class="p-6">
            <form action="{{ url('document/' . $document->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')

                <div class="space-y-2">
                    <label class="block text-sm font-medium text-gray-700">Titre (Arabe)</label>
                    <input type="text" name="titleAr" value="{{ $document->titleAr }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                </div>

                <div class="space-y-2">
                    <label class="block text-sm font-medium text-gray-700">Titre (Français)</label>
                    <input type="text" name="titleFr" value="{{ $document->titleFr }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                </div>
                    ...
                    <select name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        ...
                        @foreach ($types as $type)
                            <option value="{{ $type->type }}"
                                {{ $document->doctype->type == $type->type ? 'selected' : '' }}>
                                {{ $type->type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <input type="checkbox" id="isPublished" name="isPublished" value="1"
                        {{ $document->isPublished == 1 ? 'checked' : '' }}
